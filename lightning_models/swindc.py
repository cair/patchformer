...
class SwinDC(L.LightningModule):
    def __init__(self,
                 num_classes: int,
                 learning_rate: float,
    ...
    def forward(self, x, mask=None):
        ...
        for fidx, f in enumerate(features):
            features[fidx] = f.permute(0, 3, 1, 2)
    ...
    def calculate_metrics(self, logits, mask, step_type="train"):
        ...
        miou = iou(prediction, mask, ignore=self.num_classes)
        macc = acc(prediction, mask, ignore=self.num_classes)
