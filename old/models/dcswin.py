        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                if mask is not None:
                    ...
                    if out_index != len(self.patch_sizes) - 1 and next_cls is not None:
                        next_cls = F.softmax(next_cls, dim=1)
                        next_mask = patchify_mask(mask, self.patch_sizes[out_index + 1])
                        next_mask = check_homogeneity_classes(next_mask, self.num_patch_classes - 1, -1)

                        next_loss = patch_loss_fn(next_cls, next_mask)

                        patch_losses[out_index] = (cur_loss + next_loss) / 2
                    else:
...
class DCSwin(nn.Module):
    def __init__(self,
        ...
        self.backbone = SwinTransformer(embed_dim=embed_dim, depths=depths, num_heads=num_heads, frozen_stages=frozen_stages, num_classes=num_classes)
