...
def load_checkpoint(model,
    ...
    for table_key in relative_position_bias_table_keys:
        ...
        if table_key in model.state_dict().keys():
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            self.embed_dims.append(int(embed_dim * 2 ** i_layer))
            layer = BasicLayer(
...
def dcswin_base(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_base.pth'):
    ...
    if pretrained and weight_path is not None:
        ...
        # load_checkpoint(model, "pretrain_weights/stseg_base.pth")
    return model
...
def dcswin_small(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_small.pth'):
    ...
    if pretrained and weight_path is not None:
        ...
        #model_dict.update(old_dict)
        #model.load_state_dict(model_dict)
        load_checkpoint(model.backbone, "pretrain_weights/swin_base_patch4_window12_384_22k.pth")
    return model
...
def dcswin_tiny(pretrained=True, num_classes=4, weight_path='pretrain_weights/stseg_tiny.pth'):
    model = DCSwin(encoder_channels=(96, 192, 384, 768),
                   ...
                   frozen_stages=2)
    if pretrained and weight_path is not None:
        old_dict = load_checkpoint(weight_path)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
